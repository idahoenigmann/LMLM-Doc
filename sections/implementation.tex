...
\newline
So to conclude: A node references the nth neighbour via the nth weight. The connection of layers happens in the NeuralNetwork class with a call to NeuralNetwork::create(). Before this call is made, the layers have to be added with NeuralNetwork::add\_layer() first. As parameters this method requires the type of layer, the number of nodes in the layer and the number of connections per node to the next layer's nodes, which is regarded to as the dimension of the layer. If the dimension is one then every node has exactly one neighbour in the next layer (except for nodes in the output layer, of course. In this case the dimension is always 0). If the dimension is the number of nodes in the next layer, we speak of a dense layer, where every node of the added layer is connected to every node of the next layer.
...
int main()
{
	...
	neural_net.add_layer(2, 2, nn::core::LayerType::Input);
	neural_net.add_layer(2, 1, nn::core::LayerType::Hidden);
	neural_net.add_layer(1, 1, nn::core::LayerType::Output);
	neural_net.create();

	// Expected inputs/outputs
	std::vector<std::vector<float>> input_sets{{1.f, 1.f}, {1.f, 0.f}, {0.f, 1.f}, {0.f, 0.f}};
	std::vector<std::vector<float>> output_sets{{1.f}, {0.f}, {0.f}, {0.f}};

	std::ofstream log{"logfile.log"};
	nn::benchmark::Timer t{"AND learning"};

	nn::core::Application and_training{neural_net, input_sets, output_sets, 10000, 0.15f, log};
}
\end{lstlisting}
...
Our first goal is to find deviations of the neural network's output from expected outputs. In order to do this we first calculate the difference of the two outputs and then multiply that difference by the first derivative of the activation function applied to the output node's value of the neural network. In the C++ implementation this would just be the first derivative of the sigmoid function, and this calculation is implemented like this:

\begin{lstlisting}[language=C++]
// Calculate error of the output layer
for (std::size_t node{0}; node != nn.layers.back().nodes.size(); ++node)
{
	float output_node_value{nn.layers.back().nodes[node].value};
	float error{outputs[curr_training_set][node] - output_node_value};
	delta.front().push_back(error * math::sigmoid1(output_node_value));
}
\end{lstlisting}
...
for (std::size_t layer{nn.layers.size() - 2}; layer != 0; --layer)
{
	...
	for (std::size_t node{0}; node != nn.layers[layer].nodes.size(); ++node)
	{
		float error{0.f};
		for (unsigned next_layer_node{0}; next_layer_node != nn.layers[layer + 1].dim; ++next_layer_node)
		{
			error += delta[1][next_layer_node] * nn.layers[layer].nodes[node].weights[next_layer_node];
		}
		delta.front().push_back(error * math::sigmoid1(nn.layers[layer].nodes[node].value));
	}
}
\end{lstlisting}
