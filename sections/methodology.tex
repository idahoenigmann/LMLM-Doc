...
This code first clears the scene from other meshes (because running the script twice would place the new cube inside the old cube). Then it adds a new mesh in form of a cube at the given location. To make it more interesting the authors decided to showcase how adding some colour to the cube would work. For this to work you need a material, which is basically a specification how the surface of the object should look like. Advanced materials can represent raw or reflective surfaces, but in order to keep it simple this material will just represent a red surface (represented in red/green/blue/alpha channels ranging from 0.0 (for 0) to 1.0 (for 255)). Then the created material is applied to the object. The final result can be seen below:
